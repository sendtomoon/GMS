...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if(id.substring(id.lastIndexOf(".") + 1, id.length()).startsWith(Const.TARGET_METHOD_PREFIX)){
		//	String authSql = "select orgid,distid from jy_sys_data_auth where userid=?";
			Account curUser = AccountShiroUtil.getCurrentUser();;
			String orgSql = "select id from jy_base_org jbg start with jbg.id='"+curUser.getOrgId()+"' connect by prior jbg.id = jbg.pid";
			if(curUser!=null){	
				//String[] res = this.getAuthResByUid(connection, authSql, curUser.getAccountId());
				StringBuffer sb = new StringBuffer(boundSql.getSql());
				//String orgIds = res[0];
				//String distIds = res[1];
				//sb.insert(0, "select ts.* from (").append(") ts where ts.orgId in ").append(orgIds);
				sb.insert(0, "select ts.* from (").append(") ts where ts.orgId in (").append(orgSql).append(")");
	...
	private String getCountSql(String sql) {
		return "select count(*) as countRecord from (" + sql + ")";
	}
	 ...
	 * @return
	 */
	private String[] getAuthResByUid(Connection connection,String sql,String userId){
		...
		try{
			...
			if (rs.next()) {    
				arr[0] = rs.getString("orgid"); 
				arr[1] = rs.getString("distid");
			} 
		}catch(SQLException e){
			e.printStackTrace();
		}finally {
