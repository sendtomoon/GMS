...
public class OrgServiceImp extends BaseServiceImp<Org> implements OrgService{
	...
	public List<ZNodes> getOrgTree() {
		Account cruUser=AccountShiroUtil.getCurrentUser();
		String orgId=cruUser.getOrgId();
//		Org org=new Org();
//		org.setId(orgId);
//		org=dao.find(org).get(0);
//		if(!"4".equals(org.getOrgGrade())){
//			orgId=org.getCompanyId();
//		}
		return ((OrgDao)baseDao).getOrgTree(orgId);
	}
	...
	public List<ZNodes> getOrgTreeE4() {
		String oid = AccountShiroUtil.getCurrentUser().getOrgId();
		Org org = new Org();
		org.setId(AccountShiroUtil.getCurrentUser().getOrgId());
		List<Org> os = baseDao.find(org);
		if(os.get(0).getOrgGrade().equals(Const.ORG_GRADE_STORE)){
			return  ((OrgDao)baseDao).getOrgNode(os.get(0).getpId());
		}else{			
			return ((OrgDao)baseDao).getOrgTreeE4(oid);
		}
	}
	...
	public List<ZNodes> selectOrgTreeE4() {
		...
		org.setId(AccountShiroUtil.getCurrentUser().getOrgId());
		List<Org> os = baseDao.find(org);
		return ((OrgDao)baseDao).getOrgTree(os.get(0).getId());
	}
	@Override
	public int insertOrg(Org o) {
		int res =0;
		if(StringUtils.isNotEmpty(o.getCode())&&dao.findOrgRecordByCode(o.getCode())==0){
			dao.insert(o);
			res=1;
		}
		return res;
