...
public class ReturnBillServiceImpl extends BaseServiceImp<ReturnBill> implements ReturnBillService {
	...
	public void updateReturnBillNo(ReturnBill rb) {
		...
		// List<String> noticeFlag = rbDao.findSuJinORXiangQian(rb.getNoticeNo());//
		// 查询是否素金或镶嵌
	...
	public boolean addReturnBillFromQC(String str) {
		...
		if (noticeFlag.get(0).equals("0")) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				// rbdInsert.setPurCost((purCost.divide(actualWt,
				// 4)).multiply(unqualifyNum).toString());
				// rbdInsert.setBasicCost((basicCost.divide(actualWt,
				// 4)).multiply(unqualifyNum).toString());
				// rbdInsert.setAddCost((addCost.divide(actualWt,
				// 4)).multiply(unqualifyNum).toString());
				// rbdInsert.setOtherCost((otherCost.divide(actualWt,
				// 4)).multiply(unqualifyNum).toString());
